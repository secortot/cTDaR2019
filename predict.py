...
def get_args():
    ...
    parser.add_argument('--scale', '-s', type=float,
                        ...
                        default=0.5)
    parser.add_argument('-d', '--gpu_id', dest='gpu_id', default=2,
...
if __name__ == "__main__":
    ...
    #测试
    #fpath='/home/gzm/dq/datasets/cTDaR/test/TRACKB1/photo'
    #in_files=os.listdir(fpath)
    ...
    if not args.cpu:
        ...
        #net.load_state_dict(torch.load(args.model),map_location={'cuda:1':'cuda:2'})
        net.load_state_dict(torch.load(args.model,map_location={'cuda:1':'cuda:2'}))
    ...
    for i, fn in enumerate(in_files):
        ...
        if args.viz:
            ...
            plot_img_and_mask(img,mask_v)

        if not args.no_save:
            mask_v = mask_v.numpy()
           # mask_v=np.where(mask_v==1,mask_v, 0)
           # print(mask_v)
            result_v = mask_to_image(mask_v)
            result_v.save(out_files_v[i])
            
            mask_h = mask_h.numpy()
          #  mask_h=np.where(mask_h==1,mask_h,0)
